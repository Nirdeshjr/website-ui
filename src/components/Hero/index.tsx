...
const Hero = () => {
  ...
  useEffect(() => {
    const updateVideoBasedOnScreenWidth = () => {
      ...
      if (screenWidth < 768) {
        setIsVisible(false);
        // Set a timeout to hide the video and show the content after 7 seconds
    ...
    return () => window.removeEventListener('resize', updateVideoBasedOnScreenWidth);
  }, []);
    ...
    <>
      ...
      >
        <div className="container relative flex flex-col md:flex-row">
          ...
          {!hideVideo && (
            <div className="w-full md:w-8/12">
              <div className="w-full h-0 relative" style={{ paddingBottom: '56.25%' }}>
                <BackgroundVideo video={videoSrc.video1} className="absolute inset-0 w-full h-full" />
              </div>
            </div>
          )}
          ...
          {showContent && (
            <div className="w-full md:w-4/12 px-4 py-12 md:py-0 flex items-center" style={isVisible ? null : { marginTop: '10px' }}>
