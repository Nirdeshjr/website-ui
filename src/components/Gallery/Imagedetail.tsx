    ...
    <section className="w-full flex flex-col gap-12 py-16 lg:px-16 md:px-10 px-5">
      ...
      <main className="w-full grid lg:grid-cols-4 md:grid-cols-2 gap-x-5 gap-y-8 md:mt-8">
        {cardData.map((item, index) => (
          ...
          >
            <Img className="rounded-t-lg w-full h-[200px] overflow-hidden" image={item.src} alt={item.name} objectCover="object-cover" />
            <div className="p-5">
              <Title as="h5" className="mb-2 text-2xl font-bold tracking-tight text-white">
                {item.title}
              </Title>
              <Title as="p" className="mb-3 font-normal text-gray-400">
                {item.text}
              </Title>
            </div>
          </div>
