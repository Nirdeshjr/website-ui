      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-8 w-full md:justify-center items-start">
        {buttonCaptions.map((filter) => (
          <Button
            ...
            type="button"
            className={`focus:outline-none border-2 ${document.documentElement.classList.contains('dark') ? 'border-blue-500' : 'border-black'} ${document.documentElement.classList.contains('dark') ? 'text-white hover:border-blue-500' : 'text-black hover:bg-blue-500 hover:text-white'} font-medium rounded-lg text-sm px-5 py-2.5 capitalize ${activeFilter === filter ? 'bg-blue-500 text-white' : ''}`}
          >
